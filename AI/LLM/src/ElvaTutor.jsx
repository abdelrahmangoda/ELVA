...
function useVoiceManager({ lang, onResult }) {
  ...
  const start = useCallback(() => {
    ...
    rec.onend = () => {
      ...
      if (result.trim()) { setState("processing"); setTranscript(result); onResult(result.trim()); }
  ...
  const onPointerUp = useCallback((e) => {
    if (isMobile) return; e.preventDefault();
    if (state === "listening" && recRef.current) { try { recRef.current.stop(); } catch {} }
  }, [isMobile, state]);
  ...
  return { state, transcript, error, supported: !!SR, isMobile, start, stop, setJudging, onPointerDown, onPointerUp, onTap };
}
...
function StatCard({ label, value, sub, subColor, bar, icon, valueColor, small }) {
  return (
    <div className="elva-stat-card">
      ...
      <div className={`elva-stat-num${small?" elva-stat-num--small":""}`} style={{ color:valueColor||"#fff" }}>{value}</div>
...
export default function ElvaTutor() {
  ...
  function nextQuestion() {
    ...
    if (qIdx < lesson.quiz.length - 1) {
      ...
      setStreak(s => s+1); setXp(x => x+20+score*10); setScreen("report");
    }
  }
      ...
      <div className="elva-container">
        ...
        <header className="elva-header">
          ...
          <div className="elva-header-right">
            ...
            <button className="elva-lang-btn" onClick={()=>{ killAllAudio(); setScreen("chat"); setLesson(null); setChatHistory([]); setStudentLevel(null); setLessonTopic(null); setLang(l=>l==="ar"?"en":"ar"); }}>{t.toggleLang}</button>
              ...
              {allVoices.filter(v=>lang==="ar"?v.lang.startsWith("ar"):v.lang.startsWith("en")).map((v,i) => {
                ...
                return (
                  <button key={i} className="elva-voice-btn"
                    ...
                    onClick={()=>{ setSelectedVoice(v); stopSpeech(); const u=new SpeechSynthesisUtterance(lang==="ar"?"مرحباً! أنا إلفاَ، مدرستك الذكية. هشًرَحلك أي موضوعِ بطريقة بسيطة وممتعة!":"Hello! I'm Elva , your personal AI tutor. I can teach you any subject clearly and effectively!"); u.voice=v; u.rate=0.9; window.speechSynthesis.speak(u); }}>
            ...
            return (
              <div key={i} className="elva-msg-wrap">
                <div className={msg.role==="user"?"elva-user-row":"elva-bot-row"}>
                  ...
                  {msg.role==="user" && <div className="elva-avatar elva-avatar--user">👤</div>}
                </div>
